...
while t<T
   ...
   if cpp_accel
       ...
       tic
      [sol, bla, dt_vec] = explicitTransportVE(sol, Gt, dT, dt_vec, rock, fluidVE, ...
                                'bc', bcVE, 'wells', WVE, ...
                                'preComp', preComp);
       new_etime = toc;
       total_etime = total_etime + new_etime;
   end
   ...
   if t>= stopInject
      WVE  = []; bcVE = []; dT = 2*year();%5*year(); dTplot = dT;
   end
...
disp(['Elapsed simulation time: ', num2str(total_etime), ' seconds.']);

grav = [0, 0, 9.806649999999999];


%% Run new pressure computation
sol1 = sol;

%% Convert MATLAB data to a suitable format for the GPU solver after injection phase
[east_face_index, north_face_index] = prepareDataForGPU(sol1, Gt, rock, rock2D, fluidVE, bcVE, WVE, preComp, ...
    './SimulationData/FormationDataFromMATLABtoGPU/Johansen/data', './SimulationData/FormationDataFromMATLABtoGPU/Johansen/dimensions', ...
    './SimulationData/FormationDataFromMATLABtoGPU/Johansen/active_cells');

 save('./SimulationData/FormationDataFromMATLABtoGPU/Johansen/variablesForRunningPressureSolver.mat','grav', 'sol', 'Gt', 'SVE', 'rock', 'fluidVE', ...
     'bcVE', 'WVE','east_face_index', 'north_face_index');


%% Data for comparison with MATLAB
 [sol2, forComparison] = explicitTransportVEForComparison(sol1, Gt, 1, 0.000001, rock, fluidVE, ...
                                 'bc', bcVE, 'wells', WVE, ...
                                 'preComp', preComp);